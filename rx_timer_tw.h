...
namespace rx
{
    ...
    namespace tw
    {
        ...
        //-------------------------------------------------
        //扩展简单缓存管理器
        class entry_cache_t :protected object_cache_t<timer_item_t>
        {
            typedef object_cache_t<timer_item_t> super_t;
    ...
    class timing_wheel_t
    {
        ...
        {
            tw::timer_item_t *item = m_items_cache.alloc();
            if (!item) return NULL;
        ...
        //处理一个定时器条目
        //返回值:是否执行了定时器回调事件
        bool on_cb_item(tw::timer_item_t* item)
        {
            rx_assert(item != NULL);
