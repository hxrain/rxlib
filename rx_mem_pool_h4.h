...
namespace rx
{
    ...
    public:
        ...
        {
            if(want_size==0 || m_end_ptr == NULL || want_size & h4_node_using_mask) 
                return NULL;
        ...
        bool init( void* mem_addr,size_t mem_size )
        {
            ...
            {
                addr=size_alignx((size_t)mem_addr,h4_mem_align_size);    //给定地址向高址偏移对齐
    		    mem_size -= addr - ( size_t ) mem_addr;                 //可用尺寸相应减少
            }
        ...
        int check()
        {
            ...
            {//对整个区域进行扫描
                ...
                {//碰到有后趋的节点头了,应该是未被使用过的块
                    ...
                    if (((uint32_t)curr_block->next - (uint32_t)curr_block) != curr_block->size)
