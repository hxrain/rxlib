...
namespace rx
{
    ...
    class mem_alloc_i
    {
        ...
        typedef struct alloc_cookie_t
        {
            ...
            //---------------------------------------------
            //将附加信息放入cookie_t:Count=0代表alloc;1代表New;其他为New数组;node_mem_size代表本内存块的实际大小(>=4)
        ...
        #define __DESC_NEW_BASE__ \
            uint32_t MemSize=sizeof(VT)*Count+alloc_cookie_t::cookie_size;  \
            uint8_t *R=(uint8_t*)base_alloc(MemSize);                       \
            if (R==NULL) return NULL;                                       \
            alloc_cookie_t::set(R,Count,MemSize);                           \
            VT* Ret=(VT*)(R+alloc_cookie_t::cookie_size);                   \

        mem_alloc_i& operator=(const mem_alloc_i&);
    public:
        //-------------------------------------------------
        //根据原始分配接口,得到内存块
        void* alloc(uint32_t Size)
        {
            uint32_t MemSize=Size+alloc_cookie_t::cookie_size;
            uint8_t *R=(uint8_t*)base_alloc(MemSize);
            if (R==NULL) return NULL;
        ...
        virtual void* realloc(void* P,uint32_t NewSize,uint32_t reserved=64)
        {
            ...
            uint32_t MemSize=NewSize+reserved;
            R=(uint8_t*)base_alloc(MemSize);
            if (R==NULL) return NULL;
            alloc_cookie_t::set(R,0,MemSize);
        ...
        template<class VT> VT* New(uint32_t Count=1)
        {
            __DESC_NEW_BASE__
        ...
        template<class VT,class PT1> VT* New(PT1& P1,uint32_t Count=1)
        {
            __DESC_NEW_BASE__
        ...
        //-------------------------------------------------
        //分配一个指定类型的对象或数组,并使用参数进行构造初始化
        template<class VT,class PT1,class PT2> VT* New(PT1& P1,PT2 &P2,uint32_t Count=1)
        {
            __DESC_NEW_BASE__
        ...
        //-------------------------------------------------
        //分配一个指定类型的对象或数组,并使用参数进行构造初始化
        template<class VT,class PT1,class PT2,class PT3> VT* New(PT1& P1,PT2 &P2,PT3 &P3,uint32_t Count=1)
        {
            __DESC_NEW_BASE__
            return Count==1?ct::OC(Ret,P1,P2,P3):ct::AC(Ret,Count,P1,P2,P3);
        }
        //-------------------------------------------------
        //将New/NewEx函数分配的对象或数组进行析构并收回内存
    ...
    protected:
        ...
        //-------------------------------------------------
        //子类需给出最原始的分配接口
        virtual void* base_alloc(uint32_t size)=0;
