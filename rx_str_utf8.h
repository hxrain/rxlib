...
inline uint8_t rx_utf8_decode(const uint8_t *s, uint32_t &uc)
{
	...
	uc = s[0];
	uint8_t rc = rx_clz((~uc) & 0xFF) - 24;
	switch (rc)                      //获取首字节前导bit置位数,进行分类处理
	{
...
inline uint32_t rx_utf8_decode(const uint8_t *s, wchar_t* ustr)
{
	...
	while (*s)
	{
		...
		//进行解码错误的简化处理,要求utf8串的最后字节符合编码规范,否则只跳过首字节向后试探
		s += (sc > 1 && (s[sc - 1] & 0xC0) != 0x80) ? (uc = *s, 1) : sc;
