...
inline void test_localtime(uint64_t dt, struct tm &tp, rx_tdd_base &tdd)
{
    ...
    tdd.assert(tp.tm_year == dp->tm_year);
    tdd.assert(tp.tm_mon  == dp->tm_mon);
    tdd.assert(tp.tm_mday == dp->tm_mday);
    tdd.assert(tp.tm_hour == dp->tm_hour);
    tdd.assert(tp.tm_min  == dp->tm_min);
    tdd.assert(tp.tm_sec  == dp->tm_sec);
    tdd.assert(tp.tm_wday == dp->tm_wday);
    tdd.assert(tp.tm_yday == dp->tm_yday);

    tdd.assert(rx_make_utc(tp) == dt);
}
...
rx_tdd(localtime_base)
{
    ...
    assert(!rx_leap_year(2018));

    struct tm *dp = localtime((time_t*)&dt);

    rx_localtime(dt, tp);
    assert(tp.tm_year==2018-1900);
    assert(tp.tm_mon==5-1);
    assert(tp.tm_mday==22);
    assert(tp.tm_hour==10);
    assert(tp.tm_min==45);
    assert(tp.tm_sec==15);
    assert(tp.tm_wday==dp->tm_wday);
    assert(tp.tm_yday==dp->tm_yday);
